...
<table cellpadding="3" cellspacing="1" class="tb_data">
  ...
  <tr>
      ...
      <td><input name="content[<?=$i?>][image]" id="image_<?=$i?>" type="text" value="<?=$content[$i]['image']?>" size="30"  onfocus="loadbox.load('<?=site_url("common/upload")?>',function(json){$('#image_<?=$i?>').val(json[0].path);});"/> <a href="#" onclick="$('#image_<?=$i?>').val('');return false;">清空</a> </td>
    </tr>
    <tr>
