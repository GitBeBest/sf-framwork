...
class BaseProducts extends model
{
  ...
  public function getImages($len=0)
  {
    if($len){
    	if(function_exists("mb_substr")) return mb_substr($this->images,0,$len,"utf-8");
    	else return substr($this->images,0,$len);
    }
    return $this->images;
  }
  ...
  public function remove($addWhere = '')
  {
    ...
    $db = sf::getLib("db");
    $sql = "DELETE FROM `products` WHERE $addWhere ";
    $db->query($sql);
  ...
  public function fillObject($data=array())
  {
    ...
    isset($data["images"]) && $this->images = $data["images"];
